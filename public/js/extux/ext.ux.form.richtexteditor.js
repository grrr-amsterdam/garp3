...
Ext.ux.form.RichTextEditor = Ext.extend(Ext.form.HtmlEditor, {
     ...
     */
    defaultValue: '<p>&#8203;</p>', // (Ext.isOpera || Ext.isIE6) ? '<p>&#160;</p>' : '<p>&#8203;</p>',
	...
	getSelection: function(){
		var win = this.getWin();
		var ds = (typeof win.selection !== 'undefined' 
			? win.selection.createRange().text 
	...
	createOrRemoveTag: function(tagName){
		...
		if (!search) {
			var newNode = this.getDoc().createElement(tagName);
			...
			var newNode = this.getDoc().createTextNode(text);
	...
	addLink: function(){
		...
		var dialog = new Ext.Window({
			...
			buttons: [{
				...
				ref: '../ok',
				handler: function(){
					var url = dialog.find('name', 'url')[0].getValue(),
						title = dialog.find('name', 'title')[0].getValue(),
						target = dialog.find('name', 'target')[0].getValue() == '1';
					if (url) {
						//target == '1' ? target = 'target="_blank"' : target = '';
						!selText ? selText = url : true;
						if (currentNode) {
	...
	showMediaToolbar: function(elm){
		...
		if (elm.nodeName == 'IFRAME' || elm.nodeName == 'OBJECT') {
			this.mediaToolbarLayer.setWidth(25);
		} else {
			this.mediaToolbarLayer.setWidth((24 * 4) + 4); // 4 items in this toolbar and some space. @TODO: refine this 'formulae' if necessary
		}
		...
		var win = new Garp.ModelPickerWindow({
			...
			listeners: {
				'select': function(selected){
					...
					this.insertAtCursor('<p></p>' + Garp.videoTpl.apply({
	...
	updateStatusbar: function(str){
		...
		if (str !== '') {
			...
			var htmlPath = this.walk(sel.focusNode, true).reverse(), str = '';
    ...
    onEditorEvent : function(e){
		...
		var sel = this.getSelection();
		if(!sel) return;
		var htmlPath = this.filterTagsOnly(this.walk(this.getDoc().getElementsByTagName('body')[0], false));
		
		if((sel.focusNode && sel.focusNode.nodeName && sel.focusNode.nodeName == 'DIV')) {
			this.relayCmd('formatblock', 'p');
		} else {
			/*
			var found = false;
			Ext.each(htmlPath, function(elm){
				var t = elm.tagName.toUpperCase();
				if(t == 'P' || t == 'UL' || t == 'OL' || t == 'BLOCKQUOTE'){
					found = true;
					return;
				}
			});
			...
			Ext.each(body.childNodes, function(elm){
				if (elm.nodeName == '#text' && elm.nodeValue) {
					scope.relayCmd('formatblock', 'p');
				}
			});
			/*
			if(this.getSelection().focusNode == this.getDoc().getElementsByTagName('body')[0]){
				var range = this.getRange();
				var sel = this.getSelection();
				//sel.removeAllRanges();
				range.selectNodeContents(this.getDoc().getElementsByTagName('body')[0].firstChild);
				range.collapse(true);
				sel.removeAllRanges();
				sel.addRange(range);
			}
    ...
    updateToolbar: function(){
		...
		btns.createlink.toggle(this.getCurrentTagName() == 'a');
	...
	cleanupHtml: function(){
		...
		var strongs = Ext.DomQuery.select('em', body);
		Ext.DomHelper.useDom = false;
		Ext.each(strongs, function(s){
			var elm = Ext.get(s);
			var i = elm.dom.innerHTML;
	...
	blur: function(e, t){
		...
		if (!t || (!this.getEl().parent().contains(Ext.get(t).dom.id) && !Ext.WindowMgr.getActive())) {
			this.hasFocus = false;
