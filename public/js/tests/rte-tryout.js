Garp.Wysiwygct = Ext.extend(Ext.Panel,{
	...
	removeWysiwygBox: function(){
		Ext.get(Ext.get(window.getSelection().focusNode.parentNode).findParent('.wysiwyg-box')).remove();
		this.fixRows();
	},
	...
	setupDD: function(){
		...
		Ext.each(dragables, function(elm){
			...
			(function(){
				var w = null;
				var currentSize;
				var resizer = new Ext.Resizable(elm.id, {
					handles: 'e',
					dynamic: true,
					listeners: {
						'beforeresize': function(){
							//this.getEl().parent('.row').style('overflow','hidden');
							...
							if (others && others.elements.length) {
								...
								if (w < nw) {
									...
									if(count >= wysiwygct.maxCols || theOtherCount <= 1){
										console.log('gaat niet door! 1');
										return;
									}
									el.removeClass('grid-col-' + count + '-' + wysiwygct.maxCols);
									el.addClass('grid-col-' + (count + delta) + '-' + wysiwygct.maxCols);
									theOtherEl.removeClass('grid-col-' + theOtherCount + '-' + wysiwygct.maxCols);
