	...
	protected function _bindModels(Garp_Model_Db $model) {
		...
		foreach ($relations as $key => $config) {
			...
			if ($config['type'] === 'hasAndBelongsToMany') {
				 ...
				 * Since the subject model is NOT translated (e.g. not generated by the ModelFactory), 
				 * the $bindingModel in this case does not contain a reference to it.
				 * Because the bindingModel is internationalized, its reference is to the internationalized subject model.
				 * E.g:
				 * Model EventGroup:
				 *   referenceMap:
				 *     Event => array(
				 *       "..."
				 *       "modelClass" => "Model_EventNl"
				 *     )
				 * But Model_EventNl is not used as subject model, Model_Event is. This breaks, so let's not
				 * use the internationalized bindingModel.
 				 */
				//if ($multilingual) {
