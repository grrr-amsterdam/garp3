...
class Garp_Model_Behavior_Sluggable extends Garp_Model_Behavior_Abstract {
	...
	public function beforeInsert(array &$args) {
		...
		if (
			...
			*/
			...
			$data[$slugField] = $this->_generateUniqueSlug($baseData, $model, $slugField);
		} else {
			/**
	...
	public function generateSlug($base) {
		...
		$slug = preg_replace(array('/[^0-9a-z_-]/', '/-{2,}/'), '-', $slug);
		$slug = trim($slug, '-');
		return $slug;
	}
	...
	*/
	protected function _generateUniqueSlug($base, $model, $slugField) {
