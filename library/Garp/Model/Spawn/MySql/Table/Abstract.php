 ...
 * @subpackage Spawn
 */
abstract class Garp_Model_Spawn_MySql_Table_Abstract {
	...
	/**
	 * @param	Mixed	$columnNameOrColumn		Column name (String) or column itself (Garp_Model_Spawn_MySql_Column),
	 * 											in which case its name will be used to check the existence.
	 */
	public function columnExists($columnNameOrColumn) {
		$columnName = @get_class($columnNameOrColumn) === 'Garp_Model_Spawn_MySql_Column' ?
			$columnNameOrColumn->name :
			$columnNameOrColumn
		;
			
		if (!is_string($columnName)) {
			throw new Exception('Please feed this method either the column name as a string, or a Garp_Model_Spawn_MySql_Column instance.');
		}
	...
	public function addColumn(Garp_Model_Spawn_MySql_Column $newColumn) {
	...
	public function alterColumn(Garp_Model_Spawn_MySql_Column $newColumn) {
