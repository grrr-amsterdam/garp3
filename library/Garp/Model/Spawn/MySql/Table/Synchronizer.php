 ...
 * @subpackage MySql
 */
class Garp_Model_Spawn_MySql_Table_Synchronizer {
	/**
	...
	protected function _alterColumn(Garp_Model_Spawn_MySql_Column $sourceColumn) {
	...
	protected function _ifNullableChangesThenDeleteForeignKeys(Garp_Model_Spawn_MySql_Column $sourceColumn, array $diffProperties) {
		$source = $this->getSource();

		if (in_array('nullable', $diffProperties)) {
			foreach ($source->keys->foreignKeys as $fk) {
				if ($fk->localColumn === $sourceColumn->name) {
					Garp_Model_Spawn_MySql_ForeignKey::delete($source->name, $fk);
	...
	protected function _getDiffProperties(Garp_Model_Spawn_MySql_Column $sourceColumn) {
		$target 		= $this->getTarget();
	...
	protected function _isBindingModel(Garp_Model_Spawn_Model_Abstract $model) {
		return get_class($model) === 'Garp_Model_Spawn_Model_Binding';
	}
