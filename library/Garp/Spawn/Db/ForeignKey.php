 ...
 */
class Garp_Spawn_Db_ForeignKey extends Garp_Spawn_Db_Key {
    public $name;
    public $localColumn;
    public $remoteTable;
    public $remoteColumn;
    /**
     * MySql foreign key event string, f.i.: "ON DELETE SET NULL ON UPDATE SET NULL"
     *
     * @var string
     */
    public $events;
    ...
    public static function add($tableName, Garp_Spawn_Db_ForeignKey $key) {
        $tableName = strtolower($tableName);
        $adapter = Zend_Db_Table::getDefaultAdapter();
        $adapter->query("SET FOREIGN_KEY_CHECKS = 0;");
        $success = $adapter->query(
            "ALTER TABLE `{$tableName}` ADD CONSTRAINT `{$key->name}` FOREIGN KEY (`{$key->localColumn}`) REFERENCES `{$key->remoteTable}`(`{$key->remoteColumn}`) {$key->events};"
        );
     ...
     * @param  Garp_Spawn_Db_ForeignKey $key
     * @return bool
     */
    public static function delete($tableName, Garp_Spawn_Db_ForeignKey $key) {
        $tableName = strtolower($tableName);
        $adapter = Zend_Db_Table::getDefaultAdapter();
        $adapter->query("SET FOREIGN_KEY_CHECKS = 0;");
    ...
    public static function modify($tableName, Garp_Spawn_Db_ForeignKey $key) {
        $tableName = strtolower($tableName);
        $success   = false;
        $adapter   = Zend_Db_Table::getDefaultAdapter();
