 ...
 */
class Garp_Spawn_Sqlite_Table_Factory {
    /**
    ...
    /**
     * Produces a Garp_Spawn_Sqlite_Table_Abstract instance, based on the spawn configuration.
     *
     * @return Garp_Spawn_Sqlite_Table_Abstract
     */
    ...
    /**
     * Produces a Garp_Spawn_Sqlite_Table_Abstract instance, based on the live database.
     *
     * @return Garp_Spawn_Sqlite_Table_Abstract
     */
     ...
     * @return Garp_Spawn_Sqlite_Table_Factory
     */
    ...
    protected function _getTableClass() {
        ...
        switch (get_class($model)) {
            case 'Garp_Spawn_Model_Binding':
                return 'Garp_Spawn_Sqlite_Table_Binding';
            break;
            case 'Garp_Spawn_Model_I18n':
                return 'Garp_Spawn_Sqlite_Table_I18n';
            break;
            case 'Garp_Spawn_Model_Base':
                return 'Garp_Spawn_Sqlite_Table_Base';
            break;
            default:
    ...
    protected function _renderCreateAbstract($tableName, array $fields, array $relations, $unique) {
        ...
        foreach ($fields as $field) {
            $lines[] = Garp_Spawn_Sqlite_Column::renderFieldSql($field);
        }
        ...
        if ($primKeys) {
            $lines[] = Garp_Spawn_Sqlite_PrimaryKey::renderSqlDefinition($primKeys);
        }
        foreach ($uniqueKeys as $fieldName) {
            $lines[] = Garp_Spawn_Sqlite_UniqueKey::renderSqlDefinition($fieldName);
        }

        foreach ($relations as $rel) {
            if (($rel->type === 'hasOne' || $rel->type === 'belongsTo') && !$rel->multilingual) {
                $lines[] = Garp_Spawn_Sqlite_IndexKey::renderSqlDefinition($rel->column);
            }
        }
        ...
        foreach ($fields as $field) {
            if ($field->index) {
                $lines[] = Garp_Spawn_Sqlite_IndexKey::renderSqlDefinition($field->name);
            }
        }

        foreach ($relations as $relName => $rel) {
            if (($rel->type === 'hasOne' || $rel->type === 'belongsTo') && !$rel->multilingual) {
                $fkName = Garp_Spawn_Sqlite_ForeignKey::generateForeignKeyName($tableName, $relName);
                $lines[] = Garp_Spawn_Sqlite_ForeignKey::renderSqlDefinition(
