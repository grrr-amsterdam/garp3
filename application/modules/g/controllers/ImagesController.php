    ...
    public function viewAction() {
        ...
        if (!is_numeric($filename)) {
            throw new Zend_Controller_Action_Exception(
                'Referring to images by their filename is no longer supported ' .
                'in this url format. ' .
                'Please use the image id.',
                404
            );
        }
        if ($tpl = $request->getParam('tpl')) {
            throw new Zend_Controller_Action_Exception(
                'Template-scaled images can no longer be acquired through this dynamic url. ' .
                'Please refer to the actual location of the file.',
                404
            );
        }
